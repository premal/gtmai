...
| `agent`            | `executeAgent` — LLM w/ tool loop; `config.provider` picks the integration (`openai`/`anthropic`/`gemini`/`perplexity`); a `tavily` integration enables the web_search tool | 5                          |
