...
- `apps/worker` — BullMQ consumers (cells, signals, workflows, outbound, ads)
...
- No self-signup: users join via `/invite/[token]` links. Roles admin/editor/
  viewer; viewers are read-only outside `/auth/*`; API keys act as editor.
- `.env*` files hold secrets and are git+docker-ignored — never commit them.
- Demo login: `demo@gtmai.dev` / `demo1234` (seeded workspace "Prospects" table).
