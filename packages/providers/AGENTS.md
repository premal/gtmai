...
| `openai`, `anthropic`, `gemini`, `perplexity`, `openrouter`, `cometapi` | `<id>.chat` — LLM chat + the agent/tool loop for `agent` columns (`runAgent`); `completeChat` is the plain JSON-chat helper for metaprompt/sequence generation |
...
- `webSearch` (llm.ts) reads `credentials.tavilyApiKey` — the worker/api merge
  a workspace `tavily` integration's key into agent credentials at run time;
  without one it falls back to DuckDuckGo/Bing.
